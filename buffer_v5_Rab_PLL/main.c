...
interrupt void ADC_interrupt1(void)
{
    ...
    notch_in_pll_120 = __cospuf32(theta)*(Vc1_V); //input to the notch is sin(wt)*cos(wt+x)
    ...
    //  Vc3_ref_V = -Vc1_bpf_V; // Control equation (without Vc2 compensation)
    Vc3_ref_V = (-0.5*Vc3_rip_V*(__sinpuf32(theta+delta))) + dVc2_comp; // Control equation (with Vc2 compensation)
